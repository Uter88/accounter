...
type (
	...
	websocketService struct {
		...
		// Connections store
		connections utils.SyncMap[int64, []*websocketConn]
	}
...
func NewWebsocketService(authService authService, cfg config.Config, logger logger.Logger) *websocketService {
	return &websocketService{
		...
		connections: utils.NewSyncMap[int64, []*websocketConn](),
...
func (ws *websocketService) handleMessage(msg common.WebsocketMessage, conn *websocketConn) (resp common.WebsocketMessage, err error) {
	resp.Type = msg.Type
	resp.Payload = common.ResponseOK
	...
	switch msg.Type {
	case common.WsMessagePing, common.WsMessageAuth:
		resp.Type = common.WsMessagePong

	case common.WsMessagePong:
		resp.Type = common.WsMessagePing

	case common.WsMessageParams:
...
func (ws *websocketService) updateParams(msg common.WebsocketMessage, conn *websocketConn) error {
	conn.mu.Lock()
...
func (ws *websocketService) authorize(msg common.WebsocketMessage, conn *websocketConn) error {
	if msg.Type != common.WsMessageAuth {
		return fmt.Errorf("bad message type, have %s, want: %s", msg.Type, common.WsMessageAuth)
	}
...
func (ws *websocketService) FindAndSend(msg common.WebsocketMessage, cb func(u user.CurrentUser) bool) {
...
func (ws *websocketService) SendToUsers(msg common.WebsocketMessage, ids ...int64) {
	for _, id := range ids {
...
func (ws *websocketService) sendToConnections(msg common.WebsocketMessage, conns ...*websocketConn) {
	for _, conn := range conns {
...
func (c *websocketConn) writeMessage(msg common.WebsocketMessage) error {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *websocketConn) readMessage() (msg common.WebsocketMessage, err error) {
