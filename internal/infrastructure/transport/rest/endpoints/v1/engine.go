...
// Write blob response
func (e *v1Engine) writeBlob(c *gin.Context, format utils.FileFormat, content *bytes.Buffer) {
	switch format {
	case utils.FileFormatXLSX:
		c.Header("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

	case utils.FileFormatCSV:
		c.Header("Content-Type", "text/csv")

	case utils.FileFormatJSON:
		c.Header("Content-Type", "application/json")

	case utils.FileFormatHTML:
		c.Header("Content-Type", "text/html")

	case utils.FileFormatPDF:
		c.Header("Content-Type", "application/pdf")

	case utils.FileFormatDocX:
